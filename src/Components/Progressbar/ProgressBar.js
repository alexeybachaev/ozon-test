export class ProgressBar {
  ...
  constructor(
    ...
    this.#state.isHidden
      ? progressCircleClasses.push("progressBar__circle_hidden")
      : progressCircleClasses;
    this.#state.isAnimated
      ? progressCircleClasses.push("progressBar__circle_animated")
      : progressCircleClasses;
    ...
    addClickHandler("#animate", () =>
      this.#state.isAnimated ? this.turnAnimationOff() : this.turnAnimationOn()
    );
    addClickHandler("#hide", () =>
...
function addProgressHandler(growingSpeed = 20) {
  ...
  valueInputField.addEventListener("input", (e) => {
    ...
    clearInterval(inter);
    inter = setInterval(() => {
      currentValue < newInputValue ? currentValue++ : currentValue--;
...
function createSettingsSection(value, isAnimated, isHidden) {
  const settings = createDivWithClassAndText([
